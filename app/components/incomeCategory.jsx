  ...
  return (
    <View>
      {categories.map(category => (
        <CategoryItem key={category.id} icon="card" title={category.name} categoryId={category.id} onCategoryUpdated={fetchCategories} />
      ))}
    </View>
  );
};
...
const CategoryItem = ({ icon, title, categoryId, onCategoryUpdated }) => {
  ...
  const handleEditConfirm = async () => {
    try {
      ...
      await db.runAsync(`UPDATE income_categories SET name = ? WHERE id = ?`, [editValue, categoryId]);
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      await db.runAsync(`DELETE FROM income_categories WHERE id = ?`, [categoryId]);
      setModalDelete(false); // Hide the delete modal
      onCategoryUpdated(); // Refresh categories
    } catch (error) {
